 ...
 * Elevator capacity. The maximum number of passengers that can fit on an 
 * elevator.
 * Elevator speed. The maximum speed an elevator can travel at in feet/sec. 
 * It is assumed to be the same for all elevators.
 * Elevator acceleration. The rate at which the elevator can increase or decrease
 * speed in ft/sec2. It is assumed to be the same for all elevators. The higher the acceleration,
 * the faster the elevator can accelerate and decelerate, allowing its average speed to be faster.
 * Number of floors. The number of floors in the building, including the ground floor. It is
 ...
 * at 0 for floor 1. 
 * Floor height. The height of the floors in feet. It is assumed that each floor is 
 * the same height.
 * Number of elevators. The number of elevators in the building. Elevator numbering starts 
 ...
 * light is automatically turned off when the elevator arrives at the associated floor.
 * Elevator speed and acceleration. The elevator will automatically accelerate to the maximum 
 * speed possible until there is only enough time left to slow down to arrive at the target floor.
 * 
 * There is substantial elevator status information available:
 * Elevator speed. The current speed of the elevator. Positive speed indicates an elevator
 * heading up while negative indicates an elevator going down.
 * Elevator acceleration. The acceleration of the elevators.
 * Elevator button status. For each elevator, which floor buttons on the elevator have 
 * been pressed.
 * Elevator position. The current position of the elevator, both in feet and to 
 * the closest floor. Position is measured in feet above ground floor with zero being the bottom.
 * Elevator door status. Whether the doors for an elevator are open or closed. The 
 * status may also indicate a transition between open or closed.
 * Elevator target. The current floor target of the elevator as set by the controller. The
 * elevator will travel to that target and stop until directed to the next target.
 * Elevator committed direction. The current committed direction of the elevator. Elevators
 ...
 * to passenger travel..
 * Elevator floor service. Whether a particular elevator services a particular floor. When 
 ...
 * Elevator weight. This provides the current weight of the elevator less the weight of the 
 ...
 * Elevator target. Sets the floor target of a particular elevator. Typically, this target 
 ...
 * next target.
 * Elevator committed direction. Sets the current committed direction of the elevator. This 
 ...
 * Elevator floor service. Sets whether a particular elevator services a particular floor. 
